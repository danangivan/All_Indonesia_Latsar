...
// 2. ADD CUSTOMS MODAL UI AT THE END (before Footer)
const modalContent = `
      {/* Customs Modal */}
      {customsModalOpen && (
...
// 3. ADD TAMBAH BARANG BUTTON AND LIST TO CUSTOMS DECLARATION
...
content = content.replace(
  /onChange=\{\(value\) =>\s*handleDeclarationChange\(\s*idx,\s*"hasProhibitedGoods",\s*value,\s*\)\s*\}\s*\/>/s,
  match => match + customsListContent
);
...
`;
content = content.replace(/<div \/>\s*\}\s*<\/div>\s*<\/div>\s*<\/main>/, match => match.replace('<div />', submitBtn));
...
if (!content.includes('if (submitted) {')) {
  content = content.replace('const isDeclarationComplete = (decl: (typeof declarations)[0]) => {', successView + '\n\n  const isDeclarationComplete = (decl: (typeof declarations)[0]) => {');
}

// 5. CHANGE KIRIM TO SIMPAN
content = content.replace(/>\s*\{t\("kirim"\)\}\s*<svg/g, '>\n                                {t("simpan") || "Simpan"}\n                                <svg');

fs.writeFileSync('app/page.tsx', content, 'utf8');
console.log('Done restoring features!');
